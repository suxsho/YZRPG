...
public class battleForeverUI : MonoBehaviour {
    //-----------------------------------------------------------------
    //          无尽战斗功能
    //-----------------------------------------------------------------
    ...
    void OnGUI()
    {
        ...
        /**数值显示*/
        GUI.Label(new Rect(Screen.width / 2 - 150, 60, Screen.width - 600, Screen.height / 2 - 240), "Lv:" + monsterLevel + "抽抽怪 HP:" + monsterHP + "/" + monsterHpMax);    //怪物描述
        ...
        {
            if (GUI.Button(new Rect(Screen.width / 2 - 150, Screen.height - 100, 300, 50), "扔掉" + lostMoney + "金币退出"))
            {
                /*
    ...
    void Start()
    {
        ...
        //加成
        times = 0;
        monsterStr = monsterStr * monsterLevel * 1.04f + Random.Range(-monsterLevel * 10, monsterLevel * 10);
        monsterDef = monsterDef * monsterLevel * 1.06f + Random.Range(-monsterLevel * 10, monsterLevel * 10);
        monsterHP = Mathf.Round(monsterHP * monsterLevel * 1.05f);
        if (!buySword)
        ...
        monsterMoney = (monsterLevel + 3) + (monsterLevel * 5);
        //-----------------------------------------------------------------
        //         模拟数据，这个只是模拟的
        //-----------------------------------------------------------------
    ...
    {
        /**有体力*/
        if (gameConfig.Vit > 0 && !runAway)
        {
            battleCon = false;
            yield return new WaitForSeconds(1.0f);
            gameConfig.Vit -= 1;
            PlayerPrefs.SetInt("Vit", gameConfig.Vit);  //存档
            Application.LoadLevel(9);           //加载对应的场景
        }
        /**无体力或逃走*/
        else if(gameConfig.Vit <= 0 || runAway)
        {
            battleCon = false;
